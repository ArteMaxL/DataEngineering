...
Original file is located at
    https://colab.research.google.com/drive/1eHUiFrU2WKkWU8YgG8-rfSX6Vfm0tjdU
...
"""# Paso 2 Script creación de tablas en Amazon Redshift

Ya se encuentra en mi cuenta Amazon Redshift, este es un ejemplo de la query para crear la tabla
"""

""" CREATE TABLE IF NOT EXISTS candidatos_presidenciales_trends (
    fecha DATE,
    javier_milei NUMERIC(10,2),
    sergio_massa NUMERIC(10,2),
    patricia_bullrich NUMERIC(10,2),
    juan_schiaretti NUMERIC(10,2),
    myriam_bregman NUMERIC(10,2),
    is_partial BOOLEAN
); """

"""# Paso 3 Conexión a Redshift
Los datos para la conexión a Redshift están en un archivo externo llamado config.json
"""

import psycopg2
import json

# Leo los datos de conexión desde el archivo JSON
...
# Leo y compruebo los datos del archivo CSV original
df = iot
df.head()

# Verifico y elimino los duplicados
df.drop_duplicates(inplace=True)

# Cargo los datos en la tabla de Redshift
cur = conn.cursor()
...
for idx, row in df.iterrows():
    ...
    """, (idx, row['Javier Milei'], row['Sergio Massa'], row['Patricia Bullrich'],
...
"""# Compruebo que los datos se han cargado en Redshift"""
...
cur = conn.cursor()
cur.execute("SELECT * FROM candidatos_presidenciales_trends LIMIT 10")

# Obtengo los resultados de la consulta y los convierto a DataFrame de Pandas
results = cur.fetchall()
columns = [desc[0] for desc in cur.description]
df_test = pd.DataFrame(results, columns=columns)
