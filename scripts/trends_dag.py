...
def fetch_google_trends_data():
    ...
    keywords = ['Javier Milei', 'Sergio Massa','Patricia Bullrich','Juan Schiaretti','Myriam Bregman']
    pytrends.build_payload(keywords, cat=0, timeframe='today 5-y', geo='AR', gprop='')

    iot=pytrends.interest_over_time()
    iot

    iot.drop(columns=['isPartial']).plot(kind='line',figsize=(14,6))
...
def save_to_redshift():
    ...
    for index, row in iot.iterrows():
        ...
        query = f"INSERT INTO candidatos_presidenciales_trends (fecha, javier_milei, sergio_massa, patricia_bullrich, juan_schiaretti, myriam_bregman, is_partial) VALUES ('{fecha}', {row['Javier Milei']}, {row['Sergio Massa']}, {row['Patricia Bullrich']}, {row['Juan Schiaretti']}, {row['Myriam Bregman']}, {row['isPartial']})"
        cur.execute(query)
